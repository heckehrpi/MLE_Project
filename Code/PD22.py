...
class PlotDigitizer:
    ...
    def _quiet_init(self):
        ...
        self.cropped_preprocessed_clusters = self._preprocess_clusters(self.cropped_isolated_clusters)
    ...
    def _verbose_init(self):
        ...
        self.cropped_preprocessed_clusters = self._timed_execution("preprocessed cropped clusters in", self._preprocess_clusters, self.cropped_isolated_clusters)
    ...
    def _preprocess_clusters(self, isolated_clusters, denoise_padding=0.01, denoise_tol=0.5):
        """
    ...
    **kwargs : dict
    ...
    def _quiet_fit_data(self, epochs=25, init_split=0.25, radius_factor=0.01):
        """
    ...
    def _verbose_fit_data(self, epochs=25, init_split=0.25, radius_factor=0.01):
        """
        Executes the fit_centroids function for each detected set of data. 
        ...
        for plot in range(n_plots):
            cluster = self.cropped_preprocessed_clusters[:, :, self.cropped_layer_attributes["datasets_and_other"][plot]]
            centroids = self._timed_execution("fit centroids in", self._fit_centroids, cluster, epochs=epochs, init_split=init_split, radius_factor=radius_factor)
    ...
    **kwargs : dict
        ...
        if self._verbose:
            self._verbose_init()
            return self._verbose_fit_data(*args, **kwargs)
        else:
            self._quiet_init()
            return self._quiet_fit_data(*args, **kwargs)
