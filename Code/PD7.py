# -*- coding: utf-8 -*-
"""
Created on Mon Mar 11 10:41:19 2024
...
class PlotDigitizer:
    ...
    def __init__(self, fig_name, xlim, ylim, num_plots, verbose = False, debug=False):
        ...
        self.kmeans = KMeans(self._n_clusters, n_init=15, init='k-means++')
        print("fitting image...")
    ...
    def _fit_centroids(self, epochs=50, init_split=0.25, radius_factor=0.0075, denoise_tol=2, debug_plot=False):
    ...
    def get_centroid_coords(self):
        ...
        x = np.zeros(len(self._points), dtype=list)
        y = np.zeros(len(self._points), dtype=list)
        ...
        for i in range(len(self._points)):
            self.centroid_coords = np.zeros_like(self._centroids[i])
            self.centroid_coords[:, 0] = ((y_range[1]-y_range[0])*(max(self._x_axis) - self._centroids[i][:, 0])/height + y_range[0])
            self.centroid_coords[:, 1] = ((x_range[1]-x_range[0])*(self._centroids[i][:, 1] - min(self._y_axis))/width + x_range[0])
            
            x[i] = self.centroid_coords[:, 1]
            y[i] = self.centroid_coords[:, 0]
        return x, y