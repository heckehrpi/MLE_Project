# -*- coding: utf-8 -*-
"""
Created on Fri Mar 15 13:45:37 2024
...
class PlotDigitizer:
    ...
    def _fit_centroids(self, epochs=50, init_split=0.25, radius_factor=0.0075, denoise_tol=2, debug=False):
        if debug:
    ...
    def get_centroid_coords(self):
        coords = np.zeros(len(self._points), dtype=list)
        ...
        for i in range(len(self._points)):
            centroid_coords = np.zeros_like(self._centroids[i])
            centroid_coords[:, 0] = ((y_range[1]-y_range[0])*(max(self._x_axis) - self._centroids[i][:, 0])/height + y_range[0])
            centroid_coords[:, 1] = ((x_range[1]-x_range[0])*(self._centroids[i][:, 1] - min(self._y_axis))/width + x_range[0])
            
            coords[i] = np.fliplr(centroid_coords)
        return coords