...
@author: henhe
AutoPD v1.0: The first finalized version of PlotDigitizer! After 23 iterations, its done.
Added capability to change more of the hyperparameters when you call fit_data.
"""
...
class PlotDigitizer:
    ...
    def _fit_centroids(self, cluster, epochs=25, init_split=0.25, denoise_tol=0.5, radius_factor=0.01):
        ...
        # self.iter_plot = np.copy(cluster)
        # for i in range(np.size(centroids, 0)):
        #     self.iter_plot[centroids[i, 0], centroids[i, 1]] = 7
        #     plt.style.use("default")
        # plt.figure(dpi=500)
        # plt.imshow(self.iter_plot, cmap="Greys_r")
    ...
    def _cull_centroids(self, centroids, points_out):
        ...
        if len(inds_delete) != 0:
            # points_out = np.append(points_out, centroids[inds_delete.astype(int), :], axis=0)
    ...
    def _fit_to_centers(self, centroids, points_out, denoise_tol=0.5):
        ...
        for i in range(inds.size):
            # self.iter_plot[points_out[inds[i], 0], points_out[inds[i], 1]] = 3
